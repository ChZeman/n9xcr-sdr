...
## VHF/UHF — AntSDR E200 (AD9361)

The standard VHF/UHF node radio.
...
- Zynq-7020 SoC (dual Cortex-A9 + Artix-7 fabric): a standalone networked SDR-computer — it can
  ...
  LDMOS.
- TX output is milliwatt-class (≤ ~10 dBm) → needs a driver chain ahead of each PA.
...
- The HL2 and the AntSDR both discipline to **10 MHz**, so one 10 MHz GPSDO locks the whole
...
gives ~12 dB more HF dynamic range, and its Zynq-7020 matches the AntSDR for architectural
symmetry. The catch is it wants a 122.88 MHz reference (vs the AntSDR's 10 MHz), so the swap
