...
| `tx-chain.svg` | Transmit signal chain with part numbers: AntSDR → PGA-103+ pre-driver → CGH40010 driver → band-select → per-band finals (MRF101AN / CGH40120F) inside the control wrapper → per-band LPFs → antenna. |
